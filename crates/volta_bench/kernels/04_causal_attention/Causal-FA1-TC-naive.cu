             ...
             float *__restrict__ mrow) // [LQ]  running max per row
{
  ...
  // WMMA scratch (16x16 tiles); used only when conditions match
  __shared__ __align__(16) half wmmaA[WARPS_PER_BLOCK * 16 * 16];
  __shared__ __align__(16) half wmmaB[WARPS_PER_BLOCK * 16 * 16];
  __shared__ float wmmaC[WARPS_PER_BLOCK * 16 * 16];
  ...
  for (int k_start = 0; k_start < LK; k_start += Bc) {
    ...
    for (int r = 0; r < Br; ++r) {
      ...
      logit = (k_idx <= q_idx) ? logit : -CUDART_INF_F;
      sLogit[tid] = logit;
      ...
      const float causal_mask = (k_idx <= q_idx) ? 1.0f : 0.0f;
      w = w * causal_mask;
      sW[tid] = w;
      ...
      // ========
      // Use Tensor Cores when we have a full warp, and 16x tiles fit cleanly
      if ((VALUE_DIM % 16) == 0) {

        half *wmmaA_warp = &wmmaA[warp_id * 16 * 16];
        half *wmmaB_warp = &wmmaB[warp_id * 16 * 16];
        float *wmmaC_warp = &wmmaC[warp_id * 16 * 16];

        // Each warp owns a subset of 16-wide VALUE_DIM tiles (round-robin)
        for (int dv_base = 0; dv_base < VALUE_DIM; dv_base += 16) {
          ...
          for (int kk = 0; kk < Bc; kk += 16) {
            if (warp_id == tile_owner) {
              for (int idx = lane; idx < 16 * 16; idx += WARP_SIZE) {
                int r16 = idx / 16;
                int c16 = idx % 16;
                float wval = sW[kk + c16];
                wmmaA_warp[idx] = __float2half(wval);
                float vval = sV[kk + r16][dv_base + c16];
                wmmaB_warp[idx] = __float2half(vval);
              }
            }
            __syncthreads();
            ...
            if (warp_id == tile_owner) {
              ...
              wmma::fragment<wmma::matrix_b, 16, 16, 16, half,
                             wmma::row_major>
                  b_frag;
              wmma::load_matrix_sync(a_frag, wmmaA_warp, 16);
              wmma::load_matrix_sync(b_frag, wmmaB_warp, 16);
          ...
          if (warp_id == tile_owner) {
            wmma::store_matrix_sync(wmmaC_warp, c_frag, 16,
                                    wmma::mem_row_major);
          }
          __syncthreads();

          if (warp_id == tile_owner && lane < 16) {
            const int dv = dv_base + lane;
            const int o_off = q_idx * VALUE_DIM + dv;
            const float old_o = O[o_off];
            const float acc_tc = wmmaC_warp[lane]; // row 0, col lane
            O[o_off] = old_o * alpha + acc_tc;
          }
          __syncthreads();
        }
      }
