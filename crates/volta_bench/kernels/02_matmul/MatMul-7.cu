// Autotuned-style SGEMM adapted to match matmul_1D signature and macro pattern
// Inlined everything
...
__global__ void sgemm(const float *__restrict__ A, const float *__restrict__ B,
  ...
  // Warp-tiling parameters aligned with matmul_WT to cover 64x64 tile
  ...
  // Warp decomposition within the CTA tile
  const uint warpIdx = threadIdx.x / WARPSIZE; // warp id in block
  const uint warpCol = warpIdx % (BN / WN);
  const uint warpRow = warpIdx / (BN / WN);

  const uint threadIdxInWarp = threadIdx.x % WARPSIZE;         // [0, 31]
  const uint threadColInWarp = threadIdxInWarp % (WSUBN / TN); // [0, 7]
  const uint threadRowInWarp = threadIdxInWarp / (WSUBN / TN); // [0, 3]
