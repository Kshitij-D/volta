...
{
  __shared__ U* buf;
  // 1D-layout broadcast via shared memory (pre: intentionally missing sync)
  __shared__ U smu;
  __shared__ U sinv;
  ...
  for (auto i1=blockIdx.y; i1 < N1_ROWS; i1 += gridDim.y) {
    ...
    // Force all threads to read smu/sinv from shared without synchronization
    // to trigger a shared-memory read-after-write race under 1D layout.
    volatile U rmu = smu;
    volatile U rinv = sinv;
    const int numx = BLK_X * BLK_Y;
    const int thrx = threadIdx.x + threadIdx.y * BLK_X;
