...
//  SOURCE:
//  https://github.com/NVIDIA/Megatron-LM/commit/5478d67ef2048481e651a05053487fba029c3210
...
cuComputeGradInput(const V *__restrict__ dout, const T *__restrict__ input,
                   ...
                   U epsilon, const V *gamma, T *grad_input
                   // int has_gamma
) {
  // __requires(n2 == 1024);
  ...
  for (auto i1 = blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    // Intentionally omitted to expose race in flat 1D layout
    //__syncthreads(); // <- bug (keep commented)
  }
}

// Explicit template instantiation to force code generation
template __global__ void cuComputeGradInput<float, float, float>(
    const float *__restrict__ dout, const float *__restrict__ input,
    // const int n1,
    // const int n2,
